...
def contract_sale_asa_rwa(proxy_app_id):
    ...
    on_buy = Seq(
        number_buy.store(Gtxn[Txn.group_index() - Int(1)].asset_amount() / App.globalGet(price)),
        Assert(
            And(
                number_buy.load() >= App.globalGet(stock),
                Gtxn[Txn.group_index() - Int(1)].asset_amount() == Mul((number_buy.load()), App.globalGet(price)),
        ...
        function_contract_fees_asa(Mul((number_buy.load()), App.globalGet(price))),
        function_payment_manager(Mul((number_buy.load()), App.globalGet(price)), function_payment_asa),
        If(
